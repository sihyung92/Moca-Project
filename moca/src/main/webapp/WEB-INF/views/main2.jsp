...
<!-- Hit stores 추천 캐러샐 -->
	<c:if test="${not empty listNames }">
		<c:set var="length" value="${fn:length(listNames)}"/>
		<script type="text/javascript">console.log("storeList 의 길이는 : ${length}");</script>
	</c:if>
	<c:forEach items="${storesList}" var="bean" varStatus="status">
		<script type="text/javascript">console.log("인덱스 번호 : ${status.index}");</script>
		<c:if test="${not empty storesList[status.index]}">
		<script type="text/javascript">console.log("storesList[${status.index}]는 내용이 있습니당");</script>
			<c:set var="length" value="${fn:length(storesList[status.index])}"/>
			<c:set var="name" value="${listNames[status.index] }"/>
	 		<div class="row suggestion">
				<div class="col-md-12">
					<h5>${listNames[status.index]} <span class="glyphicon glyphicon-home" aria-hidden="true"></span></h5>
				</div>
				<div class="col-md-12 carousel slide" id="${name}" data-ride="carousel">
				  <!-- Indicators -->
				  <ol class="carousel-indicators">
				    ...
				    <c:if test="${length gt 10}">
				    	<li data-target="#${name}" data-slide-to="2"></li>
				    </c:if>
				  </ol>		
				  <!-- Wrapper for slides -->
				  <div class="carousel-inner" role="listbox">
				    <div class="item active">
				     <ul class="item-inner" style="list-style:none">
				     	<c:forEach items="${storesList[status.index]}" var="bean" begin="0" end="4" > 
				    ...
				    <div class="item">
				     <ul class="item-inner" style="list-style:none">
				     	<c:forEach items="${storesList[status.index]}" var="bean" begin="5" end="9"> 
					     	<a href="./stores/${bean.store_Id }"><li style="float:left; width:300px; height:300px;border:black 1px solid;">
					     		<img style="width:300px; height:300px;" src="${bean.storeImg1 }" alt="${bean.name }_main1">
					     		<img style="width:300px; height:300px; display:none;" src="${bean.storeImg2 }" alt="${bean.name }_main2">
					     		<img style="width:300px; height:300px; display:none;" src="${bean.storeImg3 }" alt="${bean.name }_main3">
				    ...
				    <c:if test="${length gt 10}">	
					   <div class="item">		    
					     <ul class="item-inner" style="list-style:none">
					     	<c:forEach items="${storesList[status.index]}" var="bean" begin="10" end="12"> 
						     	<a href="./stores/${bean.store_Id }"><li style="float:left; width:300px; height:300px;border:black 1px solid;">
						     		<img style="width:300px; height:300px;" src="${bean.storeImg1 }" alt="${bean.name }_main1">
					     			<img style="width:300px; height:300px; display:none;" src="${bean.storeImg2 }" alt="${bean.name }_main2">
					     			<img style="width:300px; height:300px; display:none;" src="${bean.storeImg3 }" alt="${bean.name }_main3">
